...
class GameGraphics:
    ...
    def drawCanon(self,playerNr):
        # draw the cannon
        topLeft = Point(self.game.players[playerNr].getX() - (self.game.getCannonSize() / 2), self.game.getCannonSize())
        bottomRight = Point(self.game.players[playerNr].getX() + (self.game.getCannonSize() / 2), 0)
        cannon = Rectangle(topLeft, bottomRight)
        cannon.setFill(self.game.players[playerNr].getColor())
    ...
    def drawScore(self,playerNr):
        ...
        text = Text(Point(self.game.players[playerNr].getX(), -5), f"Score: {self.game.players[playerNr].getScore()}")
        text.draw(self.win)
        return text

    def fire(self, angle, vel):
        player = self.game.getCurrentPlayer()
        proj_of_player = self.draw_projs[self.game.getCurrentPlayerNumber()]
    ...
    def updateScore(self,playerNr):
        ...
        #self.game.players[playerNr].increaseScore()
        self.draw_scores[playerNr] = Text(Point(self.game.players[playerNr].getX(), -5), f"Score: {self.game.players[playerNr].getScore()}")
        text = self.draw_scores[playerNr]
        text.draw(self.win)
